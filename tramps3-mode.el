...
The original function and arguments are available as ORIG-DIRED-DO-SHELL-COMMAND and ARGS."
  (if (tramps3-is-active)
      (let* ((current-local-files (if (dired-get-marked-files) (dired-get-marked-files)
                                    `(,(dired-get-filename))))
             (current-s3-files (--map (tramps3-local-path-to-s3-path it) current-local-files)))
        (if (seq-contains tramps3-streamable-commands (car (split-string (car args))))
            (if (-any? 'tramps3-is-directory current-local-files)
                (message (format "%s: This command cannot be applied to an s3 directory" tramps3-app-name))
              (tramps3-s3-cp-streams current-s3-files (car args)))
  ...
  (if (tramps3-is-active)
      (let* ((current-local-files (if (dired-get-marked-files) (dired-get-marked-files)
        ...
        (if (seq-contains tramps3-streamable-commands (car (split-string (car args))))
            (if (-any? 'tramps3-is-directory current-local-files)
                (message (format "%s: This command cannot be applied to an s3 directory" tramps3-app-name))
              (tramps3-s3-cp-streams current-s3-files (car args) t))
          (message (format "%s: This command is not supported by %s" tramps3-app-name tramps3-app-name))))
